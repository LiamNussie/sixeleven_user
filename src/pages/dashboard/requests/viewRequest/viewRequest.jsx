...
const ViewRequest = (props) => {
    ...
    useEffect(() => {
        axios.get(`${baseUrl}/admin/request/${id}`, config)
        .then(res => setDeets(res?.data?.result))
        // .then(res => console.log("view user", res))
    ...
    return (
        <div className="view-request">
            <div className="top">
                <Link to="/dashboard/requests" style={{textDecoration: "none"}}><p className="back"><i className="fas fa-arrow-left"></i>back</p></Link>
                <div className="btns">
                    <button style={{backgroundColor: "#ddad11"}}>Assess</button>
                    <button style={{backgroundColor: "#EB5757"}}>Deny</button>
                    <button style={{backgroundColor: "#2DA879"}}>Approve</button>
                </div>
            </div>
            <div className="prof-line">
                <div className="img" style={{backgroundImage: `url(${deets?.userId.image})`}}></div>
                <p className="name">{deets?.userId.name}</p>
                <Link style={{textDecoration: "none"}} to={{ pathname: '/dashboard/requests/view/profile', query: { data: deets} }}><p className="view">View Profile</p></Link>
            </div>
            <div className="info">
                <p className="opt">D.O.B: <strong>{deets?.userId.dateOfBirth}</strong></p>
                <p className="opt">Gender: <strong>{deets?.userId.gender}</strong></p>
                <p className="opt">Applied as: <strong>{deets?.userId.category}</strong></p>
            </div>
            <div className="reason">
                <p className="title">Reason for request</p>
                <p className='r-txt'>{deets?.reasonForAid}</p>
            </div>

            {/* <div className="skills">
                <p className="title">Skills & Certifications</p>
                <div className="sec">
                    <p className="sub-title">Highest academic Level</p>
                    <p className="val">{deets?.streetHawkerInfo?.educationLevel}</p>
                </div>
                <div className="sec">
                    <p className="sub-title">Skills</p>
                    <p className="val">Skill 1, Skill 2, Skill 3, Skill 4, Skill 5</p>
                </div>
                <div className="sec">
                    <p className="sub-title"><svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10.25 4.24985L5.37502 9.12485C5.07665 9.42322 4.90903 9.82789 4.90903 10.2499C4.90903 10.6718 5.07665 11.0765 5.37502 11.3749C5.67339 11.6732 6.07806 11.8408 6.50002 11.8408C6.92198 11.8408 7.32665 11.6732 7.62502 11.3749L12.5 6.49985C13.0968 5.90311 13.432 5.09376 13.432 4.24985C13.432 3.40594 13.0968 2.59659 12.5 1.99985C11.9033 1.40311 11.0939 1.06787 10.25 1.06787C9.40611 1.06787 8.59676 1.40311 8.00002 1.99985L3.12502 6.87485C2.22992 7.76996 1.72705 8.98398 1.72705 10.2499C1.72705 11.5157 2.22992 12.7297 3.12502 13.6249C4.02013 14.52 5.23415 15.0228 6.50002 15.0228C7.76589 15.0228 8.97992 14.52 9.87502 13.6249L14.75 8.74985" stroke="#AAC5CC" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>Certifications</p> */}
                    {/* {deets?.streetHawkerInfo?.schResult && <div className="files"> */}
                        {/* <a style={{textDecoration: "none"}} href={deets?.streetHawkerInfo?.schResult} target="_blank"><div className="filex">
                            <div className="left">
                            ...
                            <div className="txt">
                                <p className="top">Certification 1.png</p>
                        ...
                        {/* <div className="filex">
                            ...
                            <div className="txt">
                                <p className="top">Certification 1.pdf</p>
                                <p className="bottom">05/10/20</p>
                            </div>
                            </div>
                            <svg width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
           ...
           <div className="contact">
               <p className="title">Contact Person(s)</p>
               <div className="referee">
                   <p className="opt">Name: <strong>{deets?.userId?.refereeName}</strong></p>
                   <p className="opt last">Phone no:  <strong>{deets?.userId?.refereeNo}</strong></p>
                   <p className="opt">Relationship:  <strong>{deets?.userId?.refereeRelationship}</strong></p>
               </div>
           </div>
        </div>
    )
